...
import { loadRemoteFeed, readableFeedSnapshot } from "@/lib/feed-cache";
  ...
  try {
    ...
    return {
      ...
      ...(src.url.startsWith("/")
        ...
        : (() => {
            const snapshot = readableFeedSnapshot(src.url);
            return { stale: snapshot?.stale ?? false, fetchedAt: snapshot?.fetchedAt };
          })()),
    };
  } catch (err) {
...
export async function POST(req: Request) {
  ...
  if (!limited.ok) {
    // A DB outage must not initiate unmetered work. Existing public catalog
    // snapshots remain safe to serve and are explicitly marked as stale.
    if (limited.reason === "unavailable") {
      const cached = sources.map((src): CatalogPullSource => {
        const hit = readableFeedSnapshot(src.url);
        return hit
          ? { id: src.id, name: src.name, ok: true, items: slimItems(hit.feed.items),
              stale: true, fetchedAt: hit.fetchedAt }
          : { id: src.id, name: src.name, ok: false, items: [], error: "service unavailable" };
      });
      if (cached.some((source) => source.ok)) {
        if (body?.stream === true) {
          return new Response([
            JSON.stringify({ total: sources.length }),
            ...cached.map((source) => JSON.stringify({ source })),
            JSON.stringify({ done: true, degraded: "service" }), "",
          ].join("\n"), { headers: {
            "content-type": "application/x-ndjson; charset=utf-8", "cache-control": "no-store",
          } });
        }
        return NextResponse.json({ ok: true, sources: cached, degraded: "service" });
      }
    }
